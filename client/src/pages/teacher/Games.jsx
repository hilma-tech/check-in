...
class Games extends React.Component {
  ...
  removeGameFromClass = async (index) => {
    ...
    await axios.post("/api/class/removeGameRelation", {
      id: this.state.chosenGames[index].id,
    });
  };
  ...
  addGameToClass = async (index) => {
    this.setState((prevState) => {
      let newChosenGame = [
        ...prevState.chosenGames,
        prevState.gamesList[index],
      ];
      prevState.gamesList.splice(index, 1);
      return { gamesList: prevState.gamesList, chosenGames: newChosenGame };
    });
    await axios.post("/api/class/addGameRelation", {
      id: this.state.gamesList[index].id,
    });
  };
